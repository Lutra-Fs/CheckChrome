...
do
    ...
    dl_alt_base=$(sed -n '3p' ./googlechrome/tmp/parse/${i}-result.info)
    dl_file=$(sed -n '4p' ./googlechrome/tmp/parse/${i}-result.info)
    dl="https://"${dl_base}${dl_file}
    dl_alt="https://"${dl_alt_base}${dl_file}
    dl_cache=${cache}${dl_alt_base}${dl_file}
    sha256=$(sed -n '5p' ./googlechrome/tmp/parse/${i}-result.info)
